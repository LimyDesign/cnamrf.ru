...
function setTariff($tariff) {
	...
	if ($conf['db']['type'] == 'postgres')
	{
		...
		if (is_numeric($tariff)) {
			$query = "update users set tariffid = {$tariff}, qty = (select queries from tariff where id = {$tariff}) where id = {$_SESSION['userid']} and (select (sum(debet) - sum(credit)) from log where uid = {$_SESSION['userid']}) >= (select sum from tariff where id = {$tariff})";
			pg_query($query);
