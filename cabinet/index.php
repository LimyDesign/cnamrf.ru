...
function getUserList($limit = 100, $offset = 0) {
	...
	if ($_SESSION['is_admin'] == 't') {
		...
		{
			$db = pg_connect('dbname='.$conf['db']['database']) or die('Невозможно подключиться к БД: '.pg_last_error());
			$query = "select users.id, users.email, users.vk, users.ok, users.fb, users.gp, users.mr, users.ya, users.company, users.is_admin, tariff.name as tariff, (select (sum(debet) - sum(credit)) from log where uid = users.id) as balans from users left join tariff on users.tariffid = tariff.id order by balans desc limit {$limit} offset {$offset}";
			$result = pg_query($query);
